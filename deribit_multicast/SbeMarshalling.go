...
type SbeGoMarshaller struct {
	b []byte // buffer statically allocated to avoid alloc
}

func NewSbeGoMarshaller(size, cap int) *SbeGoMarshaller {
	var m SbeGoMarshaller
	m.b = make([]byte, size, cap)
	return &m
}

func (m *SbeGoMarshaller) Resize(nSize int) {
	m.b = m.b[:nSize]
}
...
// 	_m.b[0] = byte(m.BlockLength)
// 	_m.b[1] = byte(m.BlockLength >> 8)
// 	_m.b[2] = byte(m.TemplateId)
// 	_m.b[3] = byte(m.TemplateId >> 8)
// 	_m.b[4] = byte(m.SchemaId)
// 	_m.b[5] = byte(m.SchemaId >> 8)
// 	_m.b[6] = byte(m.Version)
// 	_m.b[7] = byte(m.Version >> 8)
...
func (m *SbeGoMarshaller) ReadUint64(r io.Reader, v *uint64) error {
	...
	*v = (uint64(m.b[0]) | uint64(m.b[1])<<8 |
		uint64(m.b[2])<<16 | uint64(m.b[3])<<24 |
		uint64(m.b[4])<<32 | uint64(m.b[5])<<40 |
		uint64(m.b[6])<<48 | uint64(m.b[7])<<56)
	return nil
}
...
func (m *SbeGoMarshaller) ReadInt32(r io.Reader, v *int32) error {
	...
	*v = (int32(m.b[0]) | int32(m.b[1])<<8 |
		int32(m.b[2])<<16 | int32(m.b[3])<<24)
	return nil
}
...
func (m *SbeGoMarshaller) ReadInt64(r io.Reader, v *int64) error {
	...
	*v = (int64(m.b[0]) | int64(m.b[1])<<8 |
		int64(m.b[2])<<16 | int64(m.b[3])<<24 |
		int64(m.b[4])<<32 | int64(m.b[5])<<40 |
		int64(m.b[6])<<48 | int64(m.b[7])<<56)
	return nil
}
...
func (m *SbeGoMarshaller) ReadFloat32(r io.Reader, v *float32) error {
	...
	*v = math.Float32frombits(uint32(m.b[0]) | uint32(m.b[1])<<8 |
		uint32(m.b[2])<<16 | uint32(m.b[3])<<24)
	return nil
}
...
func (m *SbeGoMarshaller) ReadFloat64(r io.Reader, v *float64) error {
	...
	*v = math.Float64frombits(uint64(m.b[0]) | uint64(m.b[1])<<8 |
		uint64(m.b[2])<<16 | uint64(m.b[3])<<24 |
		uint64(m.b[4])<<32 | uint64(m.b[5])<<40 |
		uint64(m.b[6])<<48 | uint64(m.b[7])<<56)
	return nil
